document.addEventListener('DOMContentLoaded', async () => {
    const cargarAlumnos = async () => {
        try {
            const response = await fetch('proyectonotas-production-ac16.up.railway.app//api/alumnos');
...
botonesEditar.forEach(boton => {
    boton.addEventListener('click', () => {
        ...
        botonesVerCarne.forEach(boton => {
            boton.addEventListener('click', async () => {
                ...
                try {
                    const response = await fetch('proyectonotas-production-ac16.up.railway.app//api/rellenar-pdf', {
                        method: 'POST',
                        headers: {
    ...
    const eliminarAlumno = async (id) => {
        try {
            const response = await fetch(`proyectonotas-production-ac16.up.railway.app//api/alumnos/${id}`, {
                method: 'DELETE',
            });
